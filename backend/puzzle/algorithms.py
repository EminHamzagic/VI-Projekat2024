...
class Algorithm:
    ...
    def get_legal_actions(self, state):
        ...
        legal_actions = []
        if 0 <= (up_ind := (zero_tile_ind - 3)) < max_index:
            legal_actions.append(up_ind)
        if 0 <= (right_ind := (zero_tile_ind + 1)) < max_index and right_ind % 3:
            legal_actions.append(right_ind)
        if 0 <= (down_ind := (zero_tile_ind + 3)) < max_index:
            legal_actions.append(down_ind)
        if 0 <= (left_ind := (zero_tile_ind - 1)) < max_index and (left_ind + 1) % 3:
